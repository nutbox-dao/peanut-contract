...
contract TsteemPool is Ownable {
    ...
    constructor (address _punts, address _pnutPool, address _tsteem) public {
        ...
        public
    {
        ...
        require(_amount <= tsteemBalance, "ERC20: transfer amount exceeds balance");
        ...
        if (delegators[msg.sender].tsteemAmount > 0) {
            uint256 pending = delegators[msg.sender].tsteemAmount.mul(shareAcc).div(1e12).sub(delegators[msg.sender].debtRewards);
            if(pending > 0) {
                delegators[msg.sender].availablePeanuts = delegators[msg.sender].availablePeanuts.add(pending);
            }
        }

        delegators[msg.sender].tsteemAmount = delegators[msg.sender].tsteemAmount.add(_amount);
        totalDepositedTSTEEM = totalDepositedTSTEEM.add(_amount);

        delegators[msg.sender].debtRewards = delegators[msg.sender].tsteemAmount.mul(shareAcc).div(1e12);
        ...
        onlyDelegator
    {
        ...
        delegators[msg.sender].tsteemAmount = delegators[msg.sender].tsteemAmount.sub(withdrawAmount);
    ...
    function _calculateRewardToDev() internal view returns (uint256) {
        return PnutPool.getPendingPeanuts().mul(1e11);
    }
}